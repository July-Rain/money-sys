...
// 立案模块 获取某行数据用于比较是否是当前人员
function getSelectedRowData(currentUser) {
    var grid = $("#jqGrid");
    var rowKey = grid.getGridParam("selrow");
    if (!rowKey) {
        alert("请选择一条记录");
    ...
    var rowData = grid.jqGrid('getRowData', selectedIDs);
    var checkUserList = rowData.udtId.split(',')

    function checkUser(name) {
        return name === currentUser.userId
    }

    if (checkUserList.some(checkUser) || currentUser.userId === '1') {
        return selectedIDs[0]
    } else {
        alert("非立案人员不可修改！");
        return;
...
// 现场检查模块 获取某行数据用于比较是否是当前人员
function getSelectedRowExamineData(obj, currentUser) {
    var grid = $("#jqGrid");
    if (obj) {
        grid = obj;
    }
    ...
    var rowData = grid.jqGrid('getRowData', selectedIDs);
    var checkUserList = rowData.executorName.split(',')

    function checkUserPower(name) {
        return name === currentUser.nickName
    }

    if (checkUserList.some(checkUserPower)) {
        return selectedIDs[0]
    } else {
        alert("非检查人员不可修改！");
        return;
...
//  获取某行数据用于比较是否为创建者  适用修改这些只涉及选择一条数据的时候--孙小康
function getSelectedRowUser(obj, currentUser, msg) {
    var grid = $("#jqGrid");
    var rowKey = grid.getGridParam("selrow");
    if (!rowKey) {
        alert("请选择一条记录");
    ...
    var rowData = grid.jqGrid('getRowData', selectedIDs);
    // var checkUserList = rowData;
    if (rowData.creatorName == currentUser.nickName) {
        return selectedIDs;
    }
    else {
        if (msg == null || msg == "") {
...
/**判断案件对象是否销案或移送
 * 公共方法  传入立案对象的id  返回ture or false
 * @param registrId
 * @returns {boolean}
 */
function isTranOrRvct(registrId) {
    if (!registrId) {
        return true;
    }
    var flag = false;
    $.ajax({
        type: "POST",
        url: baseURL + "mgt/caseadmin/isTranOrRvct?registrId=" + registrId,
        async: false,
        success: function (result) {
            if (result.code == 0) {
                if (result.isTran || result.isRvct) {
                    alert("该对象已被移送或撤销！")
                    flag = false;
                } else {
                    flag = true;
...
function isRvct(registrId) {
    ...
    $.ajax({
        type: "POST",
        url: baseURL + "mgt/caseadmin/isRvct?registrId=" + registrId,
        async: false,
        success: function (result) {
            if (result.code == 0) {
                if (result.isRvct) {
 ...
 * @returns {boolean}
 */
function isNotPublish(registrId) {
    var flag = false;
    $.ajax({
        type: "POST",
        url: baseURL + 'mgt/caseadmin/queryWritId?registrId=' + registrId + '&type=xzcfjd&writCode=ws10050',
        async: false,
        success: function (result) {
            if (result.code == "0") {
                if (result.writList.length == 0) {
                    flag = true;
                } else {
                    flag = false;
                    alert("该对象为不予行政处罚对象！");
 ...
 * @returns {boolean}
 */
function isImpotant(registrId) {
    var flag = false;
    $.ajax({
        type: "POST",
        url: baseURL + 'mgt/caseregistr/isImpotant?registrId=' + registrId,
        async: false,
        success: function (result) {
            if (result.code == "0") {
 ...
 * @returns {boolean}
 */
function isHaveSomeWrit(registrId, type, writCode, errorMsg) {
    var flag = false;
    $.ajax({
        type: "POST",
        url: baseURL + 'mgt/caseadmin/queryWritId?registrId=' + registrId + '&type=' + type + '&writCode=' + writCode,
        async: false,
        success: function (result) {
            if (result.code == "0") {
                if (result.writList.length == 0) {
 ...
 * @returns {boolean}
 */
function isAuth(flowList, isPass, applyTime) {
    var isAuth = false;
    if (flowList.length > 0) {
        if (isPass) {
            for (var i = 0; i < flowList.length; i++) {
                if (flowList[i].auditResultCode == "10002") {
                    //退回的数据
                    isAuth = true;
                    break;
                }
...
//现场检查  多选判断权限
//选择多条记录
function getSelectedRowsExamineData(obj, currentUser,compareId) {
    var grid = $("#jqGrid");
    if (obj) {
        grid = obj;
    }
    ...
    for (var i = 0; i < rowKey.length; i++) {
        var rowData = grid.jqGrid('getRowData', rowKey[i]);
        var checkUserList;
        if(compareId=="executorId")
        {
            checkUserList = rowData.executorId.split(',');
        }else if(compareId=="udtId")
        {
            checkUserList = rowData.udtId.split(',');
        }else if(compareId=="lastUpdPersonId"){
            checkUserList=rowData.lastUpdPersonId.split(',');
        }

        function checkUserPower(id) {
