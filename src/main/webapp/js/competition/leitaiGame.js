...
websocket.onmessage = function(event) {
    ...
    if(data.from==undefined||data.from==null||data.from==""){
        ...
        if(data.mycore!=undefined&&data.mycore!=null&&data.mycore!="")
        {
            ...
            {
                //擂主不变
                alert("对手弃权,你还是擂主，获得获胜者奖励"+data.mycore);
            }
            else
            {
                // 改变擂主

                alert("对手弃权，恭喜成为新擂主，获得获胜者奖励"+data.mycore);
            }
        ...
        if(answerpeople.length=="2")
        {
            ...
            if(data.tqList.length <= Number(nowtimu))
            {
                if(Number(vm.myscore)==Number(vm.youscore))
                {
                    ...
                    alert("全部题目答完,双方分数一样，擂主不变，平局，获得获胜奖励"+data.matchSetting.winReward+",最终得分"+vm.myscore);
                }
                ...
                else if(Number(vm.myscore)>Number(vm.youscore))
                {
                    ...
                    {
                        //擂主不变
                        alert("全部题目答完,你赢了，擂主还是你，获得获胜者奖励"+data.matchSetting.winReward+",最终得分"+vm.myscore);
                    }
                    else
                    {
                        // 改变擂主
                        $.ajax({
                            ...
                            dataType: "json",
                            data: JSON.stringify(matchSetting),
                            async: false,
                            success: function (result) {

                            }
                        });
                        alert("全部题目答完,你赢了，恭喜成为新擂主，获得获胜者奖励"+data.matchSetting.winReward+",最终得分"+vm.myscore);
                    }
